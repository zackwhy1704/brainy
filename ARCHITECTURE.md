...
| Token refresh | **Not implemented** — known gap | `AuthSessionManagerImpl` stores the access token once and never refreshes it. Supabase access tokens are short-lived (typically ≤1hr); every sync after that window 401s until app data is cleared. Cheap, well-scoped fix (store `refresh_token`, call `/auth/v1/token?grant_type=refresh_token` on 401 or proactively near `exp`) — doesn't need new UI, should land before Phase 2 leans harder on background sync. |
...
**Open, pending explicit approval — not decided, recorded here so they're not lost:** (1) local-first data reshape (raw content device-only by default, derived-data-only sync) — proposed 2026-08-30, on-device extraction confirmed not viable at minSdk 26 (AICore/Gemini Nano gates on a current-flagship hardware allowlist, not an OS version — see chat for sourcing), so the reshape would mean an ephemeral-upload extraction path, not a smaller architecture; (2) anonymous→permanent account upgrade flow (new email+OTP UI, real Phase-4-sized scope) and its trigger (proposed: 10 items captured OR 7 days since first capture, whichever first, non-blocking Home banner, never at onboarding).
